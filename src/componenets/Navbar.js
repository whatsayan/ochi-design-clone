...
const Navbar = ({ toggleNavMenu, setToggleNavMenu }) => {
  const handleToggleMenuClick = () => {
    setToggleNavMenu(prevToggleNavMenu => !prevToggleNavMenu)
  }
  return (
    <div>
      <div className='fixed z-[999] w-full px-10 py-8 font-["Neue Montreal"] flex justify-between '>
        <div>
        ...
        <div className=" hidden links sm:flex gap-10">
          {["Services", "Our Work", "About Us", "Insights", "Contact us"].map((item, index) => (
            <a href="" key={index} className={`text-lg capitalize font-light ${index === 4 && "ml-64"}`} >{item}</a>
          ))}
        </div>
        <div className="links flex gap-10 sm:hidden">
          <IoMdMenu onClick={handleToggleMenuClick} className={`text-4xl ${toggleNavMenu ? "hidden" : "block"}`} />
          <IoMdClose onClick={handleToggleMenuClick} className={`text-4xl ${toggleNavMenu ? "block" : "hidden"}`} />
        </div>
      </div>
      <div className={`${toggleNavMenu ? "block" : "hidden"} sm:hidden`}>
        <div className=' pt-28 px-5 flex flex-col min-h-screen'>
          {["Services", "Our Work", "About Us", "Insights", "Contact us"].map((item, index) => (
            <a href="" key={index} className={`text-7xl tracking-tighter capitalize font-light leading-snug`} >{item}</a>
          ))}
        </div>
      </div>
    </div>
